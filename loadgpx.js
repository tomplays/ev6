...
GPXParser.prototype.addSomeMarker = function(track, colour, width) {
    var segments = track.getElementsByTagName("trkseg");
    for(var i = 0; i < segments.length; i++) {
    ...
    for(var j = 1; j < trackpoints.length; j++) {
        ...
        var lon = parseFloat(trackpoints[j].getAttribute("lon"));
        var lat = parseFloat(trackpoints[j].getAttribute("lat"));
